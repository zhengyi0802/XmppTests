...
public class XmppServiceConnection
    ...
    private void createConnection() throws XmppStringprepException {
        Logger.debug(TAG, "creating new connection to " +
                mAccount.getHost() + ":" + mAccount.getPort());

        if (true) {
            XMPPBOSHConnection mconnect = new XMPPBOSHConnection(mAccount.getXmppJid(),
                    mAccount.getPassword(), true, mAccount.getHost(),
                    mAccount.getPort(),"/http-bind",
                    JidCreate.domainBareFrom(mAccount.getServiceName()));
            Logger.debug(TAG, "account = " + mAccount.getXmppJid());
            Logger.debug(TAG, "password = " + mAccount.getPassword());
            Logger.debug(TAG, "https = " + true);
